...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        this.initialization();
        try {
            socket = IO.socket("http://192.168.1.73:3000/"); // socket() de cho biet ket noi tu client(android) den server nao (dua vao dia chi cua server), tuong tu nhu phan PHP MySQL thi cung khong the de dia chi server la localhost
...
//    };
    private Emitter.Listener onRetrieveResult = new Emitter.Listener() { // doan code nay de lay ra gia tri cho onRetrieveResult
        ...
        public void call(Object... args) { // args la mot object hoac cung co the la mot array object nhan duoc tu server
            runOnUiThread(new Runnable() { // neu de mac dinh call(){} ben trong {} rong thi se khong thuc hien duoc gi, do do phai tao runOnUiThread() de dam bao su kien nay duoc xu ly tren UIThread thi moi tac dong den giao dien duoc
                @Override
                public void run() {
                    JSONObject jsonObject = (JSONObject) args[0]; // lay ra JSONObject nhan duoc tu server emit, o day do server chi gui 1 JSONObject nen la phan tu thu 0
                    try {
